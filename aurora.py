...
aurdatadf = pd.read_excel('annual_aurorae_nokorean_1500_1650.xlsx', dtype={"Year": int, "Observations": int})
...
M_EMAP=np.sum(aurdata[41:129])/90 
...
clengths=mins[1:]-mins[:-1]
print("clengths: ", clengths, np.mean(clengths[3:12]), np.std(clengths[3:12]))
...
with np.printoptions(precision=2):
    print("F= ", (np.sum(eventsb)+np.sum(eventsc))/(np.sum(eventsa)+np.sum(eventsd)))
    print("Contrast 1540-1627: F= ", (np.sum(eventsb[3:12])+np.sum(eventsc[3:12]))/(np.sum(eventsa[3:12])+np.sum(eventsd[3:12])))
    print("Mean rate 1540-1627: M= ", M_EMAP)
    

print("Cumulative events per quadrant: ", np.sum(eventsa[3:12]), np.sum(eventsb[3:12]), np.sum(eventsc[3:12]), np.sum(eventsd[3:12]))
...
with np.printoptions(precision=1):
    print("F: ", ((eventsb)+(eventsc))/((eventsa)+(eventsd)))
...
plt.ylim([0,maxevent])
plt.xlabel('Year')
plt.ylabel('Days with aurora / year')
...
barwidths=0.99*((np.roll(qall,-1)-qall).astype(float))
